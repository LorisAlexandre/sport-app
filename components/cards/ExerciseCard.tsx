...
export const ExerciseCard = (e: Exercise & { i: number }) => {
  const { handleExerciseSelection, exercise, exercises } =
    useUpdateWorkoutContext();

  return (
    <Button
      onClick={() => handleExerciseSelection(e.id, exercises)}
      className={`flex items-center justify-between gap-2 w-full lg:max-w-[300px] border ${
        e.id === exercise?.id ? "border-black" : "null"
      }`}
    >
      <div className="flex items-center justify-center aspect-square w-8 cursor-move">
        <svg
          width="10"
          height="18"
          viewBox="0 0 10 18"
