...
export const SerieCard = (s: Serie & { i: number }) => {
  const { serie, handleSerieSelection, handleExerciseSelection } =
    useUpdateWorkoutContext();

  return (
    <Button
      onClick={async () => {
        handleSerieSelection(s.id);
      }}
      className={`flex items-center justify-between gap-2 w-full lg:max-w-[300px] border ${
        s.id === serie.id ? "border-black" : "null"
      }`}
    >
      ...
      <div className="flex w-full justify-between items-center">
        <span>Série {s.i + 1}</span>
        <span>x {s.repetition}</span>
      </div>
    </Button>
  );
};
