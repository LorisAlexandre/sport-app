...
export const AExerciseResume = (exercise: ExerciseAnalytics) => {
    ...
    if (!!exercise.workoutTime) {
      type = (
        <span>
          ...
          {!!formatTime(exercise.workoutTime).seconds &&
            `${String(formatTime(exercise.workoutTime).seconds).padStart(
              2,
              "0"
            )}'`}
        </span>
  ...
  const renderBonusType = () => {
    ...
    switch (exercise.bonus.exerciseProp) {
      case "distance":
        type = <span>{exercise.distance} m</span>;
        break;
      case "weight":
        type = <span>{exercise.weight} kg</span>;
        break;
      case "workoutTime":
        type = (
          <span>
